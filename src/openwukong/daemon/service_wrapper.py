...
# 项目根目录（包含 config.json、start.bat 等）
PROJECT_ROOT = os.path.normpath(os.path.join(SCRIPT_DIR, "..", "..", "..", ".."))
DAEMON_SCRIPT = os.path.join(SCRIPT_DIR, "daemon.py")
PYTHON_EXE = os.path.join(PROJECT_ROOT, ".venv", "Scripts", "python.exe")
LOG_FILE = os.path.join(PROJECT_ROOT, "logs", "service_wrapper.log")
...
class ServiceWrapper:
    ...
    def _run_daemon(self) -> int:
        """启动并运行 daemon 子进程"""
        cmd = [PYTHON_EXE, "-m", "openwukong.daemon.daemon", "--target", self._target] + self._extra_args
